  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-4xl font-display font-bold tracking-tight mb-2">Reminders</h1>
          <p className="text-muted-foreground text-lg">Don't let the little things slip by.</p>
        </div>
          ...
          <CardContent className="p-6">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Register for electives" autoFocus />
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Any extra notes..." />
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-6">
              <Button variant="ghost" onClick={resetForm}>Cancel</Button>
              <Button onClick={handleCreate} disabled={!title || !remindAt || createReminder.isPending}>Save</Button>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          {isLoading ? (
            ...
            upcoming?.map(rem => {
              ...
              return (
                <div key={rem.id} className={`group flex gap-4 p-5 rounded-2xl border transition-all bg-card hover:shadow-sm ${overdue ? 'border-destructive/30 bg-destructive/5' : 'border-card-border hover:border-secondary/50'}`}>
                  <button onClick={() => toggleDone(rem)} className="mt-0.5 text-muted-foreground hover:text-primary transition-colors flex-shrink-0">
                    <Circle className="w-6 h-6" />
                  </button>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-lg mb-1 leading-tight">{rem.title}</h4>
                  ...
                  <button onClick={() => handleDelete(rem.id)} className="opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive p-2 self-start transition-opacity">
        ...
        {completed && completed.length > 0 && (
          <div className="mt-12 space-y-3 opacity-60">
            ...
            {completed.map(rem => (
              <div key={rem.id} className="flex gap-4 p-4 rounded-xl border border-transparent bg-muted/20">
                <button onClick={() => toggleDone(rem)} className="mt-0.5 text-primary flex-shrink-0">
                  <CheckCircle2 className="w-5 h-5" />
                </button>
                <div className="flex-1 min-w-0">
                  <h4 className="font-semibold text-base line-through text-muted-foreground">{rem.title}</h4>
                </div>
                <button onClick={() => handleDelete(rem.id)} className="text-muted-foreground hover:text-destructive p-1">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
            ))}
