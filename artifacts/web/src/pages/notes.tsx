...
import { FileText, Plus, Trash2, Sparkles, BookOpen } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] animate-in fade-in duration-500">
      <header className="flex justify-between items-end mb-6 shrink-0">
        <div>
          <h1 className="text-4xl font-display font-bold tracking-tight mb-2">Smart Notes</h1>
          <p className="text-muted-foreground text-lg">Jot down concepts. Let AI summarize them.</p>
        </div>
        <Button onClick={handleCreate} disabled={createNote.isPending} className="gap-2 rounded-full px-6">
      ...
      <div className="flex gap-6 flex-1 min-h-0">
        {/* Sidebar List */}
        <div className="w-1/3 flex flex-col gap-3 overflow-y-auto pr-2 pb-8">
          ...
          {notes?.length === 0 ? (
            ...
            notes?.map(note => {
              ...
              return (
                ...
                >
                  <h4 className="font-bold mb-1 truncate">{note.title || 'Untitled Note'}</h4>
                  <p className={`text-xs line-clamp-2 mb-2 ${activeNoteId === note.id ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>
                  ...
                  <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-wider">
                    ...
                    {course && (
                      <span className="flex items-center gap-1" style={{ color: activeNoteId === note.id ? 'inherit' : course.color }}>
                        <BookOpen className="w-3 h-3" /> {course.code}
                      </span>
                    )}
                  </div>
                </button>
            ...
            <>
              ...
              <div className="p-4 border-b border-border flex items-center justify-between bg-muted/20 shrink-0">
                <div className="flex gap-2">
                  <Select 
                    className="h-8 w-40 text-xs bg-background border-transparent" 
                ...
                <div className="flex items-center gap-2">
                  <Button variant="secondary" size="sm" onClick={handleSummarize} disabled={summarizeNote.isPending || !activeNote.content.trim()} className="gap-2 h-8 text-xs">
                    <Sparkles className="w-3 h-3" /> {summarizeNote.isPending ? 'Summarizing...' : 'Summarize'}
                  </Button>
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => handleDelete(activeNote.id)}>
              ...
              <div className="flex-1 overflow-y-auto flex flex-col">
                <input
                  ...
                  className="w-full bg-transparent px-8 pt-8 pb-4 text-3xl font-display font-bold outline-none placeholder:text-muted-foreground/30 shrink-0"
                />
                
                {activeNote.summary && (
                  <div className="mx-8 mb-6 p-4 bg-secondary/10 border border-secondary/20 rounded-xl shrink-0">
                ...
                <textarea
                  ...
                  className="w-full flex-1 bg-transparent px-8 pb-8 text-base outline-none resize-none placeholder:text-muted-foreground/30 leading-relaxed"
                />
              </div>
            </>
          ) : (
