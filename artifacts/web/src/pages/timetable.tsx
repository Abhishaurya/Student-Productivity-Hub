  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-display font-bold tracking-tight mb-1 md:mb-2">Weekly Timetable</h1>
          <p className="text-muted-foreground text-base md:text-lg">Your class schedule at a glance.</p>
        </div>
        <Button onClick={() => setIsCreating(!isCreating)} className="gap-2 rounded-full px-6 h-12 md:h-10 w-full md:w-auto text-base md:text-sm">
          <Plus className="w-5 h-5 md:w-4 md:h-4" /> Add Class Slot
        </Button>
      </header>
          ...
          <CardContent className="p-4 md:p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={dayOfWeek} onChange={e => setDayOfWeek(e.target.value)} className="h-11 md:h-10">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} className="h-11 md:h-10" />
                  <Input type="time" value={endTime} onChange={e => setEndTime(e.target.value)} className="h-11 md:h-10" />
                </div>
              </div>
              <div className="space-y-2 lg:col-span-1">
                <label className="text-sm font-semibold text-muted-foreground">Room / Location</label>
                <Input value={location} onChange={e => setLocation(e.target.value)} placeholder="e.g. Room 101" className="h-11 md:h-10" />
              </div>
            </div>
            <div className="flex flex-col-reverse md:flex-row justify-end gap-3 mt-6">
              <Button variant="ghost" onClick={() => setIsCreating(false)} className="h-11 md:h-10">Cancel</Button>
              <Button onClick={handleCreate} disabled={!courseId || createSlot.isPending} className="h-11 md:h-10">
                Save Slot
              </Button>
            </div>
          </CardContent>
        ...
        {slotsByDay.filter((day, i) => (i >= 1 && i <= 5) || day.slots.length > 0).map(day => (
          <div key={day.name} className="flex flex-col h-full bg-muted/20 rounded-2xl p-4 border border-border/50">
            ...
            <div className="space-y-3 flex-1">
              {day.slots.length === 0 ? (
                <div className="h-20 md:h-full flex items-center justify-center text-muted-foreground/40 font-medium text-sm">
                  Free Day
                </div>
              ) : (
                day.slots.map(slot => {
                  ...
                  return (
                    <div key={slot.id} className="relative group p-4 rounded-xl border bg-card hover:shadow-md transition-all">
                      ...
                      <button onClick={() => handleDelete(slot.id)} className="absolute top-2 right-2 opacity-100 md:opacity-0 group-hover:opacity-100 p-2 md:p-1 text-muted-foreground hover:text-destructive transition-opacity">
                        <Trash2 className="w-4 h-4 md:w-3.5 md:h-3.5" />
                      </button>
                      <h4 className="font-bold text-base md:text-sm leading-tight mb-2 pr-8">{course?.name || 'Unknown'}</h4>
                      <div className="space-y-1 text-sm md:text-xs text-muted-foreground font-semibold">
                        <div className="flex items-center gap-1.5">
                          <Clock className="w-4 h-4 md:w-3.5 md:h-3.5 opacity-70" />
                          <span>{slot.startTime} - {slot.endTime}</span>
                        </div>
                        {slot.location && (
                          <div className="flex items-center gap-1.5">
                            <MapPin className="w-4 h-4 md:w-3.5 md:h-3.5 opacity-70" />
